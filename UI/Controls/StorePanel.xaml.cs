...
public partial class StorePanel : System.Windows.Controls.UserControl
{
    ...
    /// ONE client for every cover, not one per card.
    ///
    /// — SetCoverAsync used to build an HttpClient inside itself, and the
    /// store renders every card in the section at once — 288 of them in
    /// Stable. That is 288 clients and 288 sockets opened in the same instant,
    /// which is how a .NET process runs out of ports rather than how it
    /// downloads pictures.
    private static readonly System.Net.Http.HttpClient Http = NewHttp();

    private static System.Net.Http.HttpClient NewHttp()
    {
        var h = new System.Net.Http.HttpClient
        { Timeout = TimeSpan.FromSeconds(30) };
        h.DefaultRequestHeaders.UserAgent.ParseAdd("MultiworldLauncher");
        return h;
    }

    /// At most eight covers in flight. The publishers' CDNs get a queue
    /// rather than a stampede, and a slow one cannot starve the rest.
    private static readonly System.Threading.SemaphoreSlim CoverGate = new(8);
    ...
    public void Refresh()
    {
        _coversAllowed = AskAboutCoversOnce();
        if (_index == null) _ = LoadAsync();
        else RenderCards();
    }

    /// May the library show covers? Asks once, here, the first time somebody
    /// opens it.
    ///
    /// — This question used to be asked in exactly one place: the home
    /// screen, and ONLY when an INSTALLED game was missing its cover. A fresh
    /// launcher has no games installed, so the question was never put, the
    /// answer stayed null, and every card in the library showed its initials
    /// — with no way for the player to find out why or change it. The store
    /// is the surface with eight hundred covers on it; it has to be able to
    /// ask for itself.
    ///
    /// The wording matches the home screen's, because it is the same promise:
    /// the catalogue holds addresses, never pixels, and the files land in this
    /// player's own folder.
    private bool AskAboutCoversOnce()
    {
        var s = SettingsStore.Load();
        if (s.GameArtConsent is { } already) return already;

        var ask = MessageBox.Show(Window.GetWindow(this),
            "The Plugin Library can show each game's cover.\n\n"
            + "The launcher would download them to this computer. They are not "
            + "part of the launcher and are not stored in our repository — it "
            + "only holds the addresses, and the files land in your own folder.\n\n"
            + "The artwork belongs to the games' publishers.\n\n"
            + "Download the covers?",
            "Cover art", MessageBoxButton.YesNo, MessageBoxImage.Question);

        s.GameArtConsent = ask == MessageBoxResult.Yes;
        SettingsStore.Save(s);
        return s.GameArtConsent.Value;
    }
    ...
    private async Task<bool> SetCoverAsync(Image img, StoreGame game)
    {
        try
        {
            ...
            {
                await CoverGate.WaitAsync().ConfigureAwait(true);
                try
                {
                    // Checked again inside the gate: while this card waited its
                    // turn, another card for the same game may have fetched it.
                    if (!File.Exists(local))
                    {
                        ...
                        // Written beside and moved, so a cancelled or crashed
                        // download cannot leave a half file that the cache then
                        // treats as done forever.
                        string part = local + ".part";
                        await File.WriteAllBytesAsync(part, data);
