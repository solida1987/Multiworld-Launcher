    ...
    private UIElement BuildExternalCard(ExternalSlot ext)
    {
        ...
        else
        {
            ...
            if (missing.Count == 0)
            {
                ...
                // Somebody else's server: we already know its address, the slot
                // and the password, so the tracker opens attached without the
                // player typing any of it again.
                if (TrackerButton(plugin, ext.DisplayAddress, ext.SlotName) is { } tb)
