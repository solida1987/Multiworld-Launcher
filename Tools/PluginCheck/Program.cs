...
internal static class Program
{
    ...
    private static void RunAgainstGame(IGamePlugin plugin, string gameFolder, string? slotDataFile)
    {
        ...
        try
        {
            ...
            {
                plugin.LaunchStandaloneAsync().GetAwaiter().GetResult();
                Check("game started in STANDALONE through the plugin's pipe", true);
            }
            else
            {
    ...
    private static int Main(string[] args)
    {
        ...
        // --install: run the plugin's real download path against its release.
        if (Environment.GetCommandLineArgs().Contains("--install"))
        {
            try
            {
                p.CheckForUpdateAsync().GetAwaiter().GetResult();
                Check("release lookup answers", true,
                      $"available {p.AvailableVersion ?? "?"}, installed {p.InstalledVersion ?? "none"}");
                var prog = new Progress<(int Pct, string Msg)>(r =>
                {
                    if (r.Pct % 25 == 0) Console.WriteLine($"         {r.Pct,3}%  {r.Msg}");
                });
                p.InstallOrUpdateAsync(prog).GetAwaiter().GetResult();
                bool ok = p.VerifyInstallAsync().GetAwaiter().GetResult();
                Check("game installed from the release and verifies", ok, p.GameDirectory);
            }
            catch (Exception ex)
            {
                Check("game installed from the release and verifies", false, ex.Message);
            }
        }
