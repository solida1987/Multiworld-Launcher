...
def main():
    ...
    # ---- the launcher can no longer drive an emulator by itself -----------
    #
    # BizHawk used to be compiled in. It is an extension now, like every other
    # emulator, which means a package WITHOUT it is a launcher that cannot start
    # a single game -- and it looks completely healthy until somebody presses
    # Play. So the package is where this gets checked, not the build.
    #
    # Extensions/<id>/extension.json is what BridgeRegistry reads at startup.
    if not any(n.replace("\\", "/").lower()
               .startswith("extensions/bizhawk_bridge/") for n in names):
        problems.append(
            "zip mangler Extensions/bizhawk_bridge/ — uden den kan launcheren "
            "ikke starte ET ENESTE spil (BizHawk er en extension nu)")
    elif not any(n.replace("\\", "/").lower()
                 == "extensions/bizhawk_bridge/extension.json" for n in names):
        problems.append(
            "Extensions/bizhawk_bridge/ mangler extension.json — "
            "BridgeRegistry springer mappen over uden den")
