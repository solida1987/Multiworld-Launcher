...
public static class ApworldSync
{
    ...
    public static ApworldSyncResult CopyInto(IEnumerable<string> sources, string dest)
    {
        ...
        // The engine on disk just changed, so the report ApEngine is holding
        // is now a description of a folder that no longer looks like that.
        if (copied > 0) Core.Archipelago.ApEngine.Forget();
