...
// Two access tiers:
// • FetchLatestTagAsync — HEAD redirect trick.
// against the 60 req/hr unauthenticated REST API quota.
...
// Use only in InstallOrUpdateAsync (rare user-triggered action).
...
public static class GitHubHelper
{
    ...
    /// Releases whose tag starts with a prefix, newest first.
    ///
    /// ⚠ Written because a repository can publish several release LINES from
    /// one project. FarisTheAncient/Archipelago ships both the Universal
    /// Tracker (Tracker_*) and an OSRS build (OSRSM-*), and "latest" is
    /// whichever went out most recently — which is how you download the wrong
    /// project entirely and cannot tell from the file name.
    public static async Task<System.Collections.Generic.List<GitHubRelease>>
        FetchTaggedReleasesAsync(string owner, string repo, string tagPrefix,
                                 CancellationToken ct = default)
    {
        var found = new System.Collections.Generic.List<GitHubRelease>();
        ...
        foreach (var rel in doc.RootElement.EnumerateArray())
        {
            ...
            if (rel.TryGetProperty("assets", out var arr) && arr.ValueKind == JsonValueKind.Array)
                foreach (var a in arr.EnumerateArray())
                {
                    ...
                    if (name != null && durl != null) assets.Add(new GitHubAsset(name, durl, size));
                }
            found.Add(new GitHubRelease(tag, assets));
        }
        return found;
    }
