...
public sealed class AchievementStore
{
    ...
    /// Every game this player has actually played, newest first.
    ///
    /// ⚠ Written because the Progression tab could only show a game while it
    /// was RUNNING, and a player looking at it between sessions saw nothing at
    /// all. The play log has been recording per-game time and checks the whole
    /// while; nothing was reading it back.
    public IReadOnlyList<string> PlayedGameIds()
    {
        lock (_sessions)
            return _sessions
                .Where(s => !string.IsNullOrEmpty(s.GameId))
                .GroupBy(s => s.GameId)
                .OrderByDescending(g => g.Max(s => s.EndedAt))
                .Select(g => g.Key)
                .ToList();
    }

    /// When this game was last put down. Null when it never has been.
    public DateTimeOffset? LastPlayed(string gameId)
    {
        lock (_sessions)
        {
            var mine = _sessions.Where(s => s.GameId == gameId).ToList();
            return mine.Count == 0 ? null : mine.Max(s => s.EndedAt);
        }
    }
    ...
    // ones whose threshold is met.
    // launcher-general rules always run).
...
public static class AchievementDefinitions
{
    ...
    // with a personality of their own stay here.
    ...
    // the generated per-game ladders (AchievementLadders).
