...
public sealed class LauncherUpdater
{
    ...
    {
        // Same feed format, same parser, as a plugin's own update check —
        // see VersionFeed. Reading it in two places was how the two update
        // paths would have drifted.
        var entry = await VersionFeed.ReadAsync(VersionFileUrl, ct);
        if (entry == null) return false;          // no network, or a bad feed

        LatestVersion = entry.Version;
        LatestSha256  = entry.Sha256;

        if (entry.Version > CurrentVersion)
        {
            UpdateAvailable?.Invoke(entry.Version.ToString());
            return true;
        }
        return false;
    }
