...
// IDENTITY: a SeedEntry is keyed by PatchedRomPath (the absolute .gba path).
...
public sealed record SeedEntry
{
    ...
    // The .apemerald this ROM was applied from.
...
public sealed class SeedLibraryStore
{
    ...
    // --- State ---
    // Keyed by PatchedRomPath (OrdinalIgnoreCase — Windows paths).
    ...
    // Register (or refresh) a patched ROM.
    ...
    // the seconds, and promote "new" → "in_progress".
    ...
    // Drop the registry row for a patched ROM.
