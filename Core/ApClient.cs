...
// fires, the launcher notifies the plugin and cleans up.
...
public sealed class ApClient : IAsyncDisposable
{
    ...
    // Connection tags — "AP" plus feature tags like "DeathLink".
    ...
    // Unix time of the last DeathLink Bounce WE sent.
    ...
    // Location IDs only mean something within one room, so the set records which
    // room it belongs to and is dropped when that changes — see HandleConnected.
    private readonly HashSet<long> _localChecked = new();
    private readonly object        _checkedLock  = new();
    private string?                _checkedRoomKey;

    // Serialises every outbound WS write — ClientWebSocket supports at most
    // one outstanding SendAsync (see THREADING in the header).
    ...
    // The game name for our slot, populated after Connected.
    public string? ConnectedGame { get; private set; }

    // The slot name this client authenticated with.
    // a player argument (send_location, send, hint) need it, and the session
    // is otherwise private to this class.
    public string SlotName => _session.SlotName;

    // Hint cost as a PERCENTAGE of this slot's total location count (from
    // RoomInfo / RoomUpdate, 0 = hints are free).
    ...
    // count (ConnectedChecked + ConnectedMissing).
    ...
    // multiworld seed was generated with).
    // safe to hold across packets.
    public JsonElement? SlotData { get; private set; }

    // Per-game datapackage checksums announced in RoomInfo.
    ...
    // The multiworld's seed name from RoomInfo (e.g.
    ...
    // Fires when a player completes their goal (PrintJSON type "Goal").
    public event Action<int>? GoalAnnounced;

    // Fires when a player releases their remaining items (PrintJSON type "Release").
    public event Action<int>? ReleaseAnnounced;

    // Fires when a player collects their items (PrintJSON type "Collect").
    ...
    // the "_read_hints_{team}_{slot}" key).
    // to hold past the handler.
    // finding_player, location, item, found, entrance, item_flags, status.
    public event Action<JsonElement>? HintsRetrieved;

    // Fires when the server refuses the Connect handshake.
    ...
    public async Task ConnectAsync(CancellationToken ct = default)
    {
        ...
        foreach (string uri in candidates)
        {
            ...
            // support compressed websocket connections!
    ...
    // but only broadcast NEW ones.
    // scouted location either way.
    ...
    // Safe by design: receivers dedup via their resume index.
    ...
    // Enable or disable DeathLink without reconnecting.
    ...
    private async Task HandleConnectedAsync(JsonElement el, CancellationToken ct)
    {
        ...
        // slot_info fallback for per-player GAME names.
        // array (NetworkPlayer) does NOT carry a game field in the standard
        // protocol — cross-game name resolution (hints, sent items) needs to
        // know every slot's game, and slot_info { "<slot>": { name, game, ..
        // is where the server provides it.
        if (el.TryGetProperty("slot_info", out var slotInfoEl) &&
            slotInfoEl.ValueKind == JsonValueKind.Object)
        {
            var byGame = new Dictionary<int, string>();
            foreach (var si in slotInfoEl.EnumerateObject())
            {
                if (int.TryParse(si.Name, out int slotNo) &&
                    si.Value.TryGetProperty("game", out var g) &&
                    g.ValueKind == JsonValueKind.String)
                    byGame[slotNo] = g.GetString()!;
            }
            if (byGame.Count > 0)
                Players = Players
                    .Select(p => string.IsNullOrEmpty(p.Game) && byGame.TryGetValue(p.Slot, out var pg)
                                 ? p with { Game = pg } : p)
                    .ToList();
        }
        ...
        // reconnect would replay the previous seed's checks into the new slot.
        {
            ...
            {
                lock (_checkedLock) _localChecked.Clear();
                _checkedRoomKey     = roomKey;
                _wasConnectedBefore = false;   // nothing of THIS room to replay yet
            }
        }
    ...
    private HintEntry? ParseHintEntry(JsonElement el, string rawText)
    {
        try
        {
            ...
            return new HintEntry
            {
                ...
                // Numeric identity — lets the UI resolve names through the
                // right game's DataPackage (cross-game) and lands live hints
                // on the SAME dedup key as the data-storage backlog.
                FindingSlot  = senderSlot,
