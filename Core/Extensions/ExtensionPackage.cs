...
public static class ExtensionPackage
{
    ...
    ///
    /// These are ceilings, not promises about this particular extension -- they
    /// are read before its code is ever loaded, so they say what an extension is
    /// ABLE to do. The download line is the one that matters: an extension can
    /// point the launcher at an emulator's own release, but it cannot fetch
    /// anything itself and cannot skip the dialog naming the author and licence.
    public static IReadOnlyList<string> Describe(ExtensionManifest m) => new[]
    {
        ...
        "starts a program from Emulators\\ — the emulator is never inside this package",
        "may offer to fetch that program from its author's own release — only after "
      + "showing you who wrote it and under what licence, and only if you press yes",
    };
