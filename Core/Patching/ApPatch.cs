...
public static class ApPatch
{
    ...
    public sealed record Manifest(
        ...
        // Whether archipelago.json HAS a "procedure" key at all. An absent key
        // and an unreadable value are different states: absent + delta.bsdiff4
        // is the legacy APDeltaPatch layout (worlds/Files.py assumes
        // apply_bsdiff4 there); present-but-not-a-list ("custom") is a world
        // that patches in its own code. Collapsing both into an empty step
        // list refused the first class along with the second.
        bool ProcedureDeclared);
    ...
    public static Manifest? ReadManifest(string patchPath)
    {
        try
        {
            ...
            bool procedureDeclared = r.TryGetProperty("procedure", out var proc);
    ...
    /// `knownBaseMd5` supplies the expected base-ROM hash when the container
    /// itself names none. Legacy APDeltaPatch containers keep that hash in the
    /// WORLD's code, not in archipelago.json -- the caller (a plugin whose
    /// manifest recorded the audited hash) is the only one who can provide it.
    public static Result Apply(string patchPath, string baseRomPath, string outPath,
                               IReadOnlyList<string>? knownBaseMd5 = null)
    {
        ...
        // Three container states hide behind an empty step list:
        //
        //   * "procedure" DECLARED but unreadable ("custom", OoT's .zpf): the
        //     world patches in its own code. Refuse -- the container cannot
        //     tell us what that code does.
        //   * "procedure" ABSENT + delta.bsdiff4 present: the legacy
        //     APDeltaPatch layout. worlds/Files.py's own reader assumes
        //     apply_bsdiff4(delta.bsdiff4) for exactly this state; mirror it.
        //     These containers name NO base checksum of their own (the hash
        //     lives in the world's code), so the caller must supply one -- a
        //     delta applied to the wrong base is silent garbage.
        //   * "procedure" ABSENT, no delta (APAutoPatchInterface: Pokémon
        //     Black/White, Platinum): world-side Python again. Refuse; running
        //     an empty loop would write the vanilla ROM back out and report it
        //     as patched.
        var steps = manifest.Procedure;
        bool legacyDelta = false;
        if (steps.Count == 0)
        {
            if (manifest.ProcedureDeclared)
                throw new NotSupportedException(
                    $"{Path.GetFileName(patchPath)} declares a patch procedure "
                  + "the launcher cannot read -- this game's world patches in "
                  + "its own code. Nothing was written.");
            if (files.ContainsKey("delta.bsdiff4"))
            {
                legacyDelta = true;
                steps = new[] { new Step("apply_bsdiff4", new[] { "delta.bsdiff4" }) };
            }
            else
                throw new NotSupportedException(
        ...
        var expected = manifest.BaseChecksums.Count > 0
            ? manifest.BaseChecksums
            : knownBaseMd5 ?? Array.Empty<string>();

        if (legacyDelta && expected.Count == 0)
            throw new NotSupportedException(
                $"{Path.GetFileName(patchPath)} is a legacy delta patch that "
              + "names no base-ROM checksum, and none is known for this game. "
              + "Applying it to an unverified ROM would produce silent garbage. "
              + "Nothing was written.");
        ...
        if (expected.Count > 0)
        {
            ...
            if (!expected.Contains(actual))
                throw new InvalidDataException(
                  ...
                  + (expected.Count == 1
                        ? $"The patch expects MD5 {expected[0]}\n"
                        : "The patch accepts these dumps:\n"
                          + string.Concat(expected.Select(h => $"  {h}\n")))
