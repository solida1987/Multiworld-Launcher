...
public sealed record ExternalSlot(
    ...
    /// Which named server this slot belongs to. Empty on slots saved before
    /// servers had names — ExternalServerStore.MigrateLooseSlots gives
    /// those one rather than letting them disappear from a tab that now draws
    /// by server.
...
public static class ExternalSlotStore
{
    ...
    /// Put a slot under a server. Used by the migration, which is the only
    /// thing that ever finds a slot without one.
    public static void Attach(string slotId, string serverId)
    {
        var list = All().ToList();
        int i = list.FindIndex(s => s.Id == slotId);
        if (i < 0) return;
        list[i] = list[i] with { ServerId = serverId };
        Save(list);
    }

    public static IReadOnlyList<ExternalSlot> ForServer(string serverId) =>
        All().Where(s => s.ServerId == serverId)
             .OrderBy(s => s.SlotName, StringComparer.OrdinalIgnoreCase).ToList();
