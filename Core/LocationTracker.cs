...
public sealed class LocationEntry
{
    ...
    public int    ReceiverSlot { get; set; } = -1;      // AP slot of the receiver (scouted)
    public long   ItemId       { get; set; }            // AP item id (receiver's game namespace)
...
// A named group of locations shown as one progress card (e.g.
...
public sealed class LocationTracker
{
    ...
    // Cross-game item names.
    // RECEIVING player's game (LocationInfo NetworkItem.player = receiver),
    // so "what's here" must resolve through that game's map — the flat
    // _itemNames (our game) renders other games' items as "Item #N".
    ...
    private string ResolveItemName(long itemId, int receiverSlot)
    {
        ...
        return _itemNames.GetValueOrDefault(itemId, $"Item #{itemId}");
    }
    ...
    // Refresh the player list mid-session (RoomUpdate).
    // OnConnected — that clears every entry.
    // names on scouted entries so late joiners resolve retroactively.
    public void UpdatePlayers(IReadOnlyList<ApNetworkPlayer> players)
    {
        lock (_sync)
        {
            ...
            foreach (var e in _entries.Values)
                if (e.ReceiverName.StartsWith("Slot ", StringComparison.Ordinal) &&
                    int.TryParse(e.ReceiverName.AsSpan(5), out int slot) &&
                    _playerNames.TryGetValue(slot, out var nm))
                    e.ReceiverName = nm;
        }
        Changed?.Invoke();
    }
    ...
    // isOwnGame: also feed the flat fallback maps + location groups (our
    // locations live in OUR game's namespace; other games' packages are only
    // needed for cross-game ITEM names on scouted entries).
    public void OnDataPackage(string game, JsonElement data, bool isOwnGame = true)
    {
        lock (_sync)
        {
            var gi = _gameItemNames.TryGetValue(game, out var eg) ? eg : (_gameItemNames[game] = new());

            if (data.TryGetProperty("location_name_to_id", out var locs))
                foreach (var kv in locs.EnumerateObject())
                    if (isOwnGame && kv.Value.TryGetInt64(out var id))
            ...
            // OUR game only — _nameToGroup is keyed by bare location name, so
            // a foreign game with a colliding name would overwrite our
            // category mapping (and 299 foreign games' groups are dead weight).
            if (isOwnGame && data.TryGetProperty("location_name_groups", out var groups))
    ...
    public void OnLocationInfo(ApNetworkItem[] items)
    {
        lock (_sync)
        {
            foreach (var item in items)
            {
                ...
                entry.ItemName     = ResolveItemName(item.ItemId, item.Player);
                entry.ItemId       = item.ItemId;
                entry.ItemFlags    = item.Flags;
                entry.ReceiverSlot = item.Player;
    ...
    public void OnLocationsChecked(IEnumerable<long> ids, bool addUnknown = false)
    {
        ...
        lock (_sync)
        {
            ...
            // id that isn't in it is a phantom — e.g.
            // enumeration emits base+qid+diff*1000 combinations that don't map to a
            // real location (level-milestone ids bake difficulty into the qid).
            // those instead of showing "#42282" rows.
    ...
    // Every known location id — used to scout ALL locations (checked ones
    // included) so sent-history and "what was here" survive a restart.
    public long[] GetAllIds()
    {
        lock (_sync) return _entries.Keys.ToArray();
    }

    // Snapshot of the checked-location id set (sent-history synthesis).
    public HashSet<long> GetCheckedIdSet()
    {
        lock (_sync)
            return _entries.Values.Where(e => e.IsChecked)
                                  .Select(e => e.LocationId).ToHashSet();
    }
    ...
    // The "Other" bucket always goes last.
    ...
    private void ApplyNames()
    {
        foreach (var e in _entries.Values)
        {
            ...
            // Late-arriving cross-game datapackage: re-resolve scouted item
            // names recorded as "Item #N" placeholders, precisely via the
            // stored receiver slot (ids are only unique per game).
            if (e.ReceiverSlot >= 0 && e.ItemId != 0 &&
                e.ItemName.StartsWith("Item #", StringComparison.Ordinal))
                e.ItemName = ResolveItemName(e.ItemId, e.ReceiverSlot);
        }
    }
