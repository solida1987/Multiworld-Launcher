...
public sealed record ApSlotProbeResult(
    ...
    // ⚠ The SAME Connected packet that names the game also carries this
    // slot's location lists. The probe read slot_info and threw the rest
    // away, so a slot on somebody else's server could not show "12 of 278"
    // until the player actually launched the game — and there is no spoiler
    // on this machine to fall back on. Asking is the whole answer.
    int Checked = 0,
    int Missing = 0)
{
    public int Total => Checked + Missing;
}
...
public static class ApSlotProbe
{
            ...
            foreach (var msg in doc.RootElement.EnumerateArray())
            {
                ...
                if (msg.TryGetProperty("slot_info", out var info) &&
                    ...
                    return new ApSlotProbeResult(g.GetString(), null, null,
                                                 Count(msg, "checked_locations"),
                                                 Count(msg, "missing_locations"));
    ...
    /// How many entries an array field of Connected has. Absent or the wrong
    /// shape is zero, and zero totals are shown as "unknown" rather than as a
    /// finished game.
    private static int Count(JsonElement msg, string field) =>
        msg.TryGetProperty(field, out var a) && a.ValueKind == JsonValueKind.Array
            ? a.GetArrayLength() : 0;
