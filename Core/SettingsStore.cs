...
public sealed class LauncherSettings
{
    ...
    // Folder to keep mirrored with the .apworld files the launcher installs —
    // normally Archipelago's custom_worlds.
    // Opt-in on purpose: writing into someone's Archipelago install without
    // being asked is not the launcher's business.
    ...
    // Map trackers: asked once, then respected. Same shape as cover art, and
    // for the same reason — a tracker pack is somebody else's work fetched
    // onto this machine, so it is the player's call.
    //
    // ⚠ One answer covers every game. 147 of the catalogue's games have a
    // pack, and being asked again on each of them would be the launcher
    // nagging rather than asking.
    [JsonPropertyName("tracker_consent")]
    public bool? TrackerConsent { get; set; } = null;
    ...
    // original ROM is never modified (§11).
    ...
    // keyed by plugin GameId.
    // picker after the plugin validated the folder.
    ...
    // Per-event notification sound choice: event key ("progression", "trap",
    // "death", "goal") → a bundled sound name from Assets/Sounds (without
    // extension), or "system" for the old Windows sound, or "none" to mute
    // just that event. Missing key = the shipped default for that event.
    [JsonPropertyName("notification_sound_map")]
    public Dictionary<string, string> NotificationSoundMap { get; set; } = new();
