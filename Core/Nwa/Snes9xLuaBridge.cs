...
// poll()/is_goal_complete()/receive_item() contract each tick.
// byte-for-byte the one already mock-verified on BizHawk — one logic source, two
// transports (§14 / EMULATOR_MATRIX §6.1).
// interpreter (no native/packed blob → AV-neutral).

// PERFORMANCE: a single poll() does 200+ memory reads (e.g.
...
public sealed class BridgeConfig
{
    ...
    // slot_data option flags the module reads (e.g.
...
public sealed class Snes9xLuaBridge : IDisposable
{
    ...
    // The domain argument (index `i`) → NWA region.
