...
```json
{
  ...
  "minLauncher": "3.4.0",
  "update": {
    "versionUrl": "https://raw.githubusercontent.com/you/yourgame/main/plugin_version.txt",
    "packageUrl": "https://github.com/you/yourgame/releases/latest/download/mygame.londonplugin"
  }
}
```
...
| `minLauncher` | optional. The oldest launcher your build runs on — see §9 |
| `update` | optional. Where you publish newer builds — see below |
...
### Telling players about your updates

Add an `update` block and the launcher will check, on start-up, whether you have
published something newer — and if so, offer it. Leave it out and it never
checks; that is the default, because a check is a network call to your server
and it is your call whether to make one happen.

`versionUrl` points at a two-line text file **you** host:

```
1.2.0
9f2c…the SHA-256 of the .londonplugin, 64 hex digits
```

That is the same format the launcher uses for its own updates. Both URLs must be
`https`, and both are served **by your project** — the launcher never
redistributes your plugin, and adding this block does not put your code in
anyone else's hands.

What the player sees:

1. A prompt naming your plugin, the installed version, the new version, and the
   host it would fetch from. They can say no, and nothing happens.
2. If they say yes: the package is downloaded and checked against the checksum
   in your feed. A mismatch is discarded, not installed.
3. Then the ordinary consent dialog again — with the new file's hash, and with
   **any line in `declares` your new version added marked as new**.

That third step is the reason this is safe to offer. An update that starts
downloading from somewhere it did not before cannot slip past looking like a
bug-fix. Keep `declares` honest across versions and your players will have
nothing to be surprised by.

Bump `version` in `plugin.json` and the version line in your feed together. The
launcher compares them, so a feed that runs ahead of the package offers an
update that installs the same thing the player already has.
...
does not hold. A plugin built against launcher 3.0.1 will not load on 3.0.0.

Say so with `minLauncher`. Without it, a player on an older launcher gets the
raw .NET binding failure — *"Could not load file or assembly 'Multiworld
Launcher, Version=3.3.0.0'"* — which tells them nothing about what to do. With
it, they are told to update the launcher first — and a downloaded update that
needs a newer launcher is refused at that point rather than half-installed. (The
feed carries only a version and a checksum, so the launcher cannot know your
requirement until it has read the package; the prompt still appears, but nothing
is replaced.)

Set it to the launcher you built against, and only raise it when you actually
start using something newer.
