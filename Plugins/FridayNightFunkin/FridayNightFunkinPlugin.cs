...
namespace LauncherV2.Plugins.FridayNightFunkin;

// ═══════════════════════════════════════════════════════════════════════════════
// FridayNightFunkinPlugin — launch integration for "Friday Night Funkin'",
// a free PC rhythm game with an Archipelago world. No Steam release —
// itch.io / standalone.
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * No Steam release — free standalone download. GameDirectory = string.Empty
//     default; user sets install dir via Settings.
//   * AP game string: "Friday Night Funkin'"
//   * ConnectsItself = true: built-in AP client handles the server connection.
//   * SupportsStandalone = true: can run without AP.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class FridayNightFunkinPlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "Archipelago-Games";
    private const string GH_REPO     = "Friday-Night-Funkin";
    ...
    public string GameId       => "friday_night_funkin";
    public string DisplayName  => "Friday Night Funkin'";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "Friday Night Funkin'";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "friday_night_funkin.png");

    public string ThemeAccentColor => "#FF3C6E";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "Friday Night Funkin' is a free open-source rhythm game where Boyfriend must " +
        "battle rival after rival in rap battles to win the heart of his Girlfriend. " +
        "In the Archipelago randomizer, songs and weeks are shuffled into the multiworld " +
        "pool. Free standalone download — no Steam required.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Download Friday Night Funkin' from itch.io or the GitHub releases page " +
            "and extract it to your preferred folder, then set the path in the Settings panel."));
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download Friday Night Funkin' from itch.io or the GitHub releases page (link below).",
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title = "Select Friday Night Funkin' folder",
        ...
        {
            ("Friday Night Funkin' GitHub ↗",   REPO_URL),
            ("Friday Night Funkin' releases ↗", REPO_URL + "/releases"),
    ...
    private void StartGame()
    {
        ...
        if (Directory.Exists(dir))
        {
            ...
            {
                "FridayNightFunkin.exe", "Friday Night Funkin.exe",
                "fnf.exe", "FNF.exe", "funkin.exe",
            })
            {
            ...
            if (exe == null)
            {
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        ...
                        if (stem.IndexOf("funkin", StringComparison.OrdinalIgnoreCase) >= 0 ||
                            stem.IndexOf("fnf",    StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch Friday Night Funkin'. Make sure the game is installed.", ex);
        }
    }
