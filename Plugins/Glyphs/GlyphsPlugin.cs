...
public sealed class GlyphsPlugin : IGamePlugin
{
    ...
    public string GameId       => "glyphs";
    public string DisplayName  => "GLYPHS";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "GLYPHS";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "glyphs.png");

    public string ThemeAccentColor => "#9B7FD4";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "GLYPHS is a PC puzzle game that challenges players to decipher ancient symbol " +
        "patterns and unlock hidden meanings. In the Archipelago randomizer, puzzle " +
        "solutions and unlocks are shuffled into the multiworld pool. " +
        "Standalone download — no Steam required.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Download GLYPHS from the GitHub releases page and extract it to " +
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download GLYPHS from the GitHub releases page (link below).",
        ...
        {
            ("GLYPHS GitHub ↗",   REPO_URL),
            ("GLYPHS releases ↗", REPO_URL + "/releases"),
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "Glyphs.exe", "GLYPHS.exe", "glyphs.exe" })
            {
                ...
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        if (Path.GetFileNameWithoutExtension(f)
                                .IndexOf("glyph", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch GLYPHS. Make sure the game is installed.", ex);
        }
    }
