...
namespace LauncherV2.Plugins.Touhou185;

// ═══════════════════════════════════════════════════════════════════════════════
// Touhou185Plugin — launch integration for "Touhou 18.5: Violet Detector Arrangement"
// (PC native, community AP world) with the Archipelago randomizer.
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * No Steam release — user must browse to the game folder manually.
//   * AP world repo: furret78/Archipelago
//   * ConnectsItself = false: requires external AP TextClient.
//   * SupportsStandalone = false: requires an AP room.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class Touhou185Plugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "furret78";
    private const string GH_REPO     = "Archipelago";
    private const string GH_RELEASES = "https://api.github.com/repos/furret78/Archipelago/releases";
    private const string REPO_URL    = "https://github.com/furret78/Archipelago";
    ...
    public string GameId      => "touhou_18_5";
    public string DisplayName => "Touhou 18.5";
    public string Subtitle    => "PC · AP client required";
    public string ApWorldName => "Touhou 18.5";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "touhou_18_5.png");

    public string ThemeAccentColor => "#C84040";

    public string[] GameBadges => new[] { "Free Download" };

    public string Description =>
        "Touhou 18.5: Violet Detector Arrangement is a community Archipelago world " +
        "for Touhou Luna Nights/18.5. Download the AP world from GitHub and follow " +
        "setup instructions.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "1. Download the game/AP world from the GitHub releases page. " +
            "2. Extract to a folder of your choice. " +
            "3. Click Browse to set the game folder. " +
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download the game/AP world from the GitHub releases page (link below).",
            "2. Extract to a folder of your choice.",
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title = "Select Touhou 18.5 game folder",
    ...
    private void StartGame()
    {
        ...
        {
            // Known exe name for Touhou 18.5
            string candidate = Path.Combine(dir, "th185.exe");
            if (File.Exists(candidate)) exe = candidate;

            // Fuzzy fallback: any exe containing "th185"
            if (exe == null)
            {
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        ...
                        if (stem.IndexOf("th185", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
