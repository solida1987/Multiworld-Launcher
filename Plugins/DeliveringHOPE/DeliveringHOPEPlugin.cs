...
namespace LauncherV2.Plugins.DeliveringHOPE;

// ═══════════════════════════════════════════════════════════════════════════════
// DeliveringHOPEPlugin — launch integration for "Delivering HOPE", a standalone
// Archipelago puzzle game (community AP fork, StellatedCUBE/Delivering-Hope-Archipelago).
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * Free standalone download from GitHub releases. GameDirectory = string.Empty
//     default; user sets install dir via Settings.
//   * AP game string: "Delivering HOPE"
//   * ConnectsItself = true: built-in AP client handles the server connection.
//   * SupportsStandalone = false: requires an AP room.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class DeliveringHOPEPlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "StellatedCUBE";
    private const string GH_REPO     = "Delivering-Hope-Archipelago";
    ...
    public string GameId       => "delivering_hope";
    public string DisplayName  => "Delivering HOPE";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "Delivering HOPE";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "delivering_hope.png");

    public string ThemeAccentColor => "#2090D0";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "Delivering HOPE is a standalone Archipelago puzzle game. The built-in " +
        "client connects to the AP server directly. Download from GitHub releases, " +
        "extract, and browse to the folder.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Download Delivering HOPE from the GitHub releases page and extract it to " +
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download Delivering HOPE from the GitHub releases page (link below).",
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title = "Select Delivering HOPE folder",
        ...
        {
            ("Delivering HOPE GitHub ↗",   REPO_URL),
            ("Delivering HOPE releases ↗", REPO_URL + "/releases"),
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "DeliveringHOPE.exe", "Delivering HOPE.exe",
                                            "delivering_hope.exe", "DeliveringHope.exe" })
            {
            ...
            if (exe == null)
            {
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        ...
                        if (stem.IndexOf("delivering", StringComparison.OrdinalIgnoreCase) >= 0 ||
                            stem.IndexOf("hope",       StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch Delivering HOPE. Make sure the game is installed.", ex);
        }
    }
