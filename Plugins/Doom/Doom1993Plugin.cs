...
namespace LauncherV2.Plugins.Doom;

// ═══════════════════════════════════════════════════════════════════════════════
// Doom1993Plugin — install / update / launch for "DOOM (1993)" — the original
// id Software DOOM played through APDOOM, a fork of Crispy Doom with a BUILT-IN
...
//
// This is the FIRST native-PC game added in a new wave and is intentionally
// modelled on Plugins/SoH/SoHPlugin.cs — same shape: GitHub-release install/
// update, bring-your-own asset (here a DOOM.WAD instead of a ROM), launch with
// AP connection prefill, ConnectsItself = true, SupportsStandalone = true.
//
...
// one as primary and the original as a pinned fallback:
//
...
//     so install still works offline-of-the-primary.
//
// HOW IT CONNECTS (VERIFIED against the official Archipelago "DOOM 1993" setup
// guide, https://archipelago.gg/tutorial/DOOM%201993/setup_en, and AP PR #3757
...
//   verified path (cleaner than SoH's config-file guesswork):
//       crispy-apdoom -game doom -apserver <server:port> -applayer <slot>
//       [-password <pw>] [-skill <1-5>] [-apdeathlinkoff]
//   The CLI exe is "crispy-apdoom(.exe)". Windows builds ALSO ship a GUI
//   front-end ("apdoom-launcher.exe" on Daivuk, "APDoomLauncher.exe" on the new
//   org) that auto-detects WADs and offers the same connection fields. AP allows
//   one connection per slot, so — like SoH/OpenTTD — the launcher must NOT hold
...
//   DOOM.WAD (id's IWAD). Verified setup: copy DOOM.WAD into the extracted
//   APDOOM folder next to the exe. This plugin lets the user pick their WAD,
//   validates it by CONTENT (the IWAD magic — first 4 bytes "IWAD" — plus a
//   plausible size), copies it into the launcher's own ROM library
//   (Games/ROMs/doom_1993/, original NEVER modified, §11), and stages a copy
//   named DOOM.WAD next to the exe so APDOOM finds it. When launching the CLI
//   exe directly we also pass "-iwad <stagedWad>" defensively.
//   Known IWAD sizes (doomwiki.org): shareware DOOM1.WAD 4,196,020 B;
//   registered DOOM.WAD v1.9 11,159,840 B; Ultimate Doom v1.9 12,408,292 B. We
//   accept a loose 3–20 MB window so the common shareware/registered/ultimate/
//   BFG/Unity dumps all pass — APDOOM itself is the authoritative validator at
//   load. We DO reject a PWAD (mod patch) here, since the base game needs an
//   IWAD.
//
// THE AP WORLD:
//   game string "DOOM 1993" (verified: worlds/doom_1993/__init__.py →
//   DOOM1993World.game = "DOOM 1993"). The new org's release also ships a
//   "doom_1993_beta.apworld" (a beta of the same world); the Daivuk release
//   shipped "doom_1993.apworld". This plugin fetches whichever apworld the
...
//     JSON sidecar (Games/ROMs/doom_1993/doom1993_launcher.json) rather than in
...
public sealed class Doom1993Plugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────
    ...
    private const string VersionFileName = "apdoom_version.dat";

    /// Canonical filename APDOOM expects for the IWAD next to the exe.
    private const string StagedWadName = "DOOM.WAD";

    // ── IGamePlugin — Identity ────────────────────────────────────────────────

    public string GameId      => "doom_1993";
    public string DisplayName => "DOOM (1993)";
    public string Subtitle    => "Native PC · built-in Archipelago";

    /// EXACT AP game string — verified against worlds/doom_1993/__init__.py
    /// (DOOM1993World.game = "DOOM 1993").
    public string ApWorldName => "DOOM 1993";

    public string IconPath =>
        Path.Combine(AppContext.BaseDirectory, "Assets", "doom_1993.png");

    public string ThemeAccentColor => "#8A1A12";   // DOOM red
    public string[] GameBadges     => new[] { "Requires DOOM.WAD" };

    public string Description =>
        "The original 1993 DOOM by id Software, played through APDOOM — a fork of " +
        "Crispy Doom with a built-in Archipelago client. Weapons, keycards, level " +
        "access, powerups and more are shuffled into the multiworld, and the engine " +
        "connects to the Archipelago server itself — no emulator, no Lua bridge. " +
        "APDOOM ships no game data: you must supply your own DOOM.WAD (from your " +
        "Steam, GOG, or original copy of DOOM). The launcher copies your WAD next " +
        "to the game and connects you on launch.";
    ...
    /// Root folder where APDOOM is installed.
    public string GameDirectory { get; set; }
        = Path.Combine(AppContext.BaseDirectory, "Games", "Doom1993");
    ...
    /// Where the release's doom_1993 apworld is saved for the user to copy into
    ...
    private string ApWorldLocalPath
    {
        get
        {
            ...
            return Path.Combine(GameDirectory,
                string.IsNullOrEmpty(name) ? "doom_1993.apworld" : name);
        }
    }

    private string VersionFilePath => Path.Combine(GameDirectory, VersionFileName);

    /// The launcher's own ROM-library copy of the user's DOOM.WAD (§11).
    ...
    private string SettingsSidecarPath
        => Path.Combine(RomLibraryDirectory, "doom1993_launcher.json");
...
#pragma warning restore CS0067
        ...
        CancellationToken ct = default)
    {
        ...
        if (apworldUrl != null)
        {
            try
            {
                progress.Report((85, "Downloading the DOOM 1993 apworld..."));
        ...
        progress.Report((100,
            $"APDOOM {version} ready. Pick your DOOM.WAD in Settings if you have " +
    ...
    public Task LaunchAsync(ApSession session, CancellationToken ct = default)
    {
        ...
        // Make sure the IWAD is staged next to the exe (APDOOM needs DOOM.WAD).
        StageWadForGame();
    ...
    public Task LaunchStandaloneAsync(CancellationToken ct = default)
    {
        ...
        // No AP args — plain DOOM. Still pass -iwad when launching the CLI engine
        // directly so it finds the staged WAD without a chooser.
    ...
    public UIElement? CreateSettingsPanel()
    {
        ...
        panel.Children.Add(new TextBlock
        {
            Text = "DOOM.WAD", FontSize = 10, FontWeight = FontWeights.SemiBold,
        ...
        panel.Children.Add(new TextBlock
        {
            Text = "APDOOM needs your own DOOM.WAD (the IWAD from your Steam, GOG, or " +
                   "original copy of DOOM). The launcher copies it into its own folder " +
                   "and stages a copy named DOOM.WAD next to the game — your original " +
                   "file is never modified. Shareware DOOM1.WAD also works for episode 1.",
        ...
        foreach (var (label, url) in new[]
        {
            ...
            ("DOOM 1993 Setup Guide ↗",    "https://archipelago.gg/tutorial/DOOM%201993/setup_en"),
    ...
    /// pattern, excluding linux/mac/source) and the doom_1993 apworld. Asset
        ...
        PickWindowsAndApworld(JsonElement assets)
    {
        ...
        foreach (var a in assets.EnumerateArray())
        {
            ...
            // doom_1993 apworld (prefer the 1993 world; ignore doom_ii/heretic).
            if (lower.EndsWith(".apworld") &&
                (lower.Contains("doom_1993") || lower.Contains("doom1993")))
            {
                apworld     = url;
                apworldName = name;
            }
    ...
    ///   -game doom -apserver <host:port> -applayer <slot> [-password <pw>]
    ///   [-iwad <stagedWad>] [-skill <n>] [-fullscreen]
    /// (Setup guide + AP PR #3757.) Slot/password are quoted to survive spaces.
    ...
    /// persist the COPY's path, and stage it as DOOM.WAD next to the exe if the
    /// game is installed. Returns true when a WAD was imported.
    private bool PromptForWadFile()
    {
        var dlg = new Microsoft.Win32.OpenFileDialog
        {
            Title  = "Select your DOOM.WAD",
            Filter = "DOOM IWAD (*.wad)|*.wad|All files (*.*)|*.*",
        };
        if (dlg.ShowDialog() != true) return false;

        string? bad = ValidateDoomWad(dlg.FileName);
        if (bad != null)
        {
            MessageBox.Show(bad, "Not a valid DOOM IWAD",
    ...
    /// Content check for a DOOM IWAD: the first 4 bytes must be the ASCII magic
    /// "IWAD" (a "PWAD" mod patch is rejected — the base game needs an IWAD), and
    /// the size must be in a loose 3–20 MB window (known IWADs: shareware
    /// 4,196,020 B; registered 11,159,840 B; ultimate 12,408,292 B; the BFG/
    /// Unity DOOM.WAD is a bit larger but still well under 20 MB). APDOOM itself
    /// is the authoritative validator at load — we only catch obvious mistakes.
    /// Returns null when acceptable, else a short human-readable reason.
    private static string? ValidateDoomWad(string path)
    {
        try
        {
            long len = new FileInfo(path).Length;
            const long min = 3L  * 1024 * 1024;
            const long max = 20L * 1024 * 1024;
            if (len < min)
                return "That file is too small to be a DOOM IWAD. Pick your DOOM.WAD " +
                       "(about 4 MB shareware, or 11–12 MB for the full game).";
            if (len > max)
                return "That file is too large to be a DOOM IWAD. Pick your DOOM.WAD, " +
                       "not a mod megawad.";
    ...
    /// Copy the user's library WAD to DOOM.WAD next to the exe so APDOOM finds
        ...
        CancellationToken ct)
    {
        string tempZip = Path.Combine(Path.GetTempPath(),
            $"apdoom-{version}-{Guid.NewGuid():N}.zip");
        try
        {
