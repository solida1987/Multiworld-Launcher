...
public sealed class D2RandomizerSettings
{
    ...
    // The slider counts NORMAL skills only.
    // all-or-nothing pack: tick it and every new skill is added on top of the
    // slider's value — set the slider to 60 and tick the box and you get 60
    // normal skills PLUS the whole pack.
    // widen a slider to make the feature work.
    // The slider value goes to the game AS SET.
    // to max whenever the experimental pack was on, which silently ignored the
    // player's choice (slider at 1 still produced every vanilla skill — the
    // exact bug Maegis reported).
    // the game adds it itself, so the launcher must not second-guess the value.
    // The mod treats "the full pool" and "no limit" as the same thing, so an
    // unticked box simply sends the vanilla count rather than a second flag the
