...
namespace LauncherV2.Plugins.CornKidz64;

// ═══════════════════════════════════════════════════════════════════════════════
// CornKidz64Plugin — launch integration for "Corn Kidz 64", an indie PC game
// with an Archipelago world. No Steam release — itch.io / standalone.
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * No Steam release — standalone download. GameDirectory = string.Empty by
//     default; user sets install dir via Settings.
//   * AP game string: "Corn Kidz 64"
//   * ConnectsItself = true: built-in AP client handles the server connection.
//   * SupportsStandalone = true: can run without AP.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class CornKidz64Plugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "alwaysintreble";
    private const string GH_REPO     = "Archipelago-Corn-Kidz-64";
    ...
    public string GameId       => "corn_kidz_64";
    public string DisplayName  => "Corn Kidz 64";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "Corn Kidz 64";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "corn_kidz_64.png");

    public string ThemeAccentColor => "#F5C842";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "Corn Kidz 64 is an indie PC platformer inspired by classic N64 collect-a-thons. " +
        "Explore vibrant worlds, gather corn, and defeat bosses in this retro-styled " +
        "adventure. The Archipelago randomizer shuffles collectibles and progression " +
        "items into the multiworld pool. Free standalone download — no Steam required.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Download Corn Kidz 64 from itch.io or the GitHub releases page and " +
            "extract it to your preferred folder, then set the path in the Settings panel."));
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download Corn Kidz 64 from itch.io or the GitHub releases page (link below).",
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title = "Select Corn Kidz 64 folder",
        ...
        {
            ("Corn Kidz 64 GitHub ↗",   REPO_URL),
            ("Corn Kidz 64 releases ↗", REPO_URL + "/releases"),
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "CornKidz64.exe", "Corn Kidz 64.exe", "cornkidz64.exe" })
            {
                ...
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        if (Path.GetFileNameWithoutExtension(f)
                                .IndexOf("corn", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch Corn Kidz 64. Make sure the game is installed.", ex);
        }
    }
