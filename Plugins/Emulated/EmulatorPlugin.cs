...
public abstract class EmulatorPlugin : IGamePlugin
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (!IsEmulatorPresent)
        {
            ...
            throw new FileNotFoundException(
                ...
                $"This launcher does not download emulators. Get {backend.DisplayName} " +
                $"from {backend.HomepageUrl} and extract it into:" + nl +
    ...
    public virtual UIElement? CreateSettingsPanel()
    {
        ...
        if (!emuPresent)
        {
            ...
            panel.Children.Add(new TextBlock
            {
                Text = $"The launcher does not download emulators. Get {backend.DisplayName} " +
                       $"from {backend.HomepageUrl} and extract it into the folder below, so " +
                       $"that {backend.ExeName} sits directly in it.",
                FontSize     = 11,
                Foreground   = muted,
                TextWrapping = TextWrapping.Wrap,
                Margin       = new Thickness(0, 0, 0, 8),
        ...
        string subdir, string displayName, string homepage, string exeName)
    {
        try
        {
            ...
            File.WriteAllText(readme, string.Join(Environment.NewLine, new[]
            {
                ...
                "",
                "The launcher does not download emulators. Get " + displayName,
                "yourself from " + homepage + " and extract it here, so that",
                exeName + " sits directly in this folder:",
                "",
                "    " + dir,
                "    " + Path.Combine(dir, exeName),
                "",
                "Nothing else is needed. Delete this file once you have done it.",
            }));
        }
