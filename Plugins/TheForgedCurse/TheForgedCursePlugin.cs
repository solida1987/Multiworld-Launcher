...
namespace LauncherV2.Plugins.TheForgedCurse;

// ═══════════════════════════════════════════════════════════════════════════════
// TheForgedCursePlugin — launch integration for "The Forged Curse", a PICO-8
// dungeon crawler with Archipelago integration by cheesepak.
//
// ── FACTS ─────────────────────────────────────────────────────────────────────
//   * PICO-8 game. Releases at: https://github.com/cheesepak/ap-forged-curse
...
//
//   * AP game string: "The Forged Curse"
...
//       1. forged_curse.html  — open in default browser
//       2. the_forged_curse.html — open in default browser
//       3. *.html (any HTML file in GameDirectory) — open first one found
//       4. forged_curse.p8    — launch PICO-8 with it (UseShellExecute)
//       5. *.p8 (any cart in GameDirectory)
//       6. Fallback: open GitHub releases page in browser
//
...
public sealed class TheForgedCursePlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "cheesepak";
    private const string GH_REPO     = "ap-forged-curse";
    private const string GH_RELEASES = "https://api.github.com/repos/cheesepak/ap-forged-curse/releases";
    private const string REPO_URL    = "https://github.com/cheesepak/ap-forged-curse";
    private const string RELEASES_URL = "https://github.com/cheesepak/ap-forged-curse/releases";
    ...
    public string GameId      => "the_forged_curse";
    public string DisplayName => "The Forged Curse";
    public string Subtitle    => "PICO-8 · AP client required";
    public string ApWorldName => "The Forged Curse";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "the_forged_curse.png");

    public string ThemeAccentColor => "#FF8040";   // PICO-8 warm orange

    public string[] GameBadges => new[] { "PICO-8", "Free Download" };

    public string Description =>
        "The Forged Curse is a PICO-8 dungeon crawler with Archipelago integration. " +
    ...
    // ── AP bridge events ──────────────────────────────────────────────────────
    // The Forged Curse does not connect to the AP server itself — the player
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((50, "Opening The Forged Curse releases page..."));
        try
        {
        ...
        progress.Report((100,
            "Download The Forged Curse from the GitHub releases page. You can play " +
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title = "Select The Forged Curse folder",
        ...
        panel.Children.Add(new System.Windows.Controls.TextBlock
        {
            Text = "The Forged Curse does not have a built-in AP client. Use the " +
        ...
        {
            ("The Forged Curse on GitHub ↗",    REPO_URL),
    ...
    ///   Priority 1: forged_curse.html
    ///   Priority 2: the_forged_curse.html
    ///   Priority 3: any *.html in the directory
    ///   Priority 4: forged_curse.p8
    ///   Priority 5: any *.p8 in the directory
    ...
    private string? ResolveGameFile()
    {
        ...
        // Try preferred HTML names first.
        foreach (string name in new[] { "forged_curse.html", "the_forged_curse.html" })
        {
            string path = Path.Combine(dir, name);
            if (File.Exists(path)) return path;
        }

        // Any HTML file in the directory (case-insensitive).
        try
        {
            foreach (string f in Directory.EnumerateFiles(dir, "*.html"))
                return f;   // return the first one found
        }
        catch { /* directory access failure — non-fatal */ }

        // Try preferred .p8 cart names.
        foreach (string name in new[] { "forged_curse.p8", "the_forged_curse.p8" })
        {
            string path = Path.Combine(dir, name);
            if (File.Exists(path)) return path;
        }
