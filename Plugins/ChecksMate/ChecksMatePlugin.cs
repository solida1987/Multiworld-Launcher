...
namespace LauncherV2.Plugins.ChecksMate;

// ═══════════════════════════════════════════════════════════════════════════════
// ChecksMatePlugin — launch integration for "ChecksMate", a free standalone
// chess Archipelago game (community AP world, alwaysintreble/ChecksMate).
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * No Steam release — free standalone download. GameDirectory = string.Empty
//     default; user sets install dir via Settings.
//   * AP game string: "ChecksMate"
//   * ConnectsItself = true: built-in AP client handles the server connection.
//   * SupportsStandalone = false: requires an AP room.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class ChecksMatePlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "alwaysintreble";
    private const string GH_REPO     = "ChecksMate";
    ...
    public string GameId       => "checksmate";
    public string DisplayName  => "ChecksMate";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "ChecksMate";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "checksmate.png");

    public string ThemeAccentColor => "#A0785A";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "ChecksMate is a chess-inspired Archipelago game where piece captures and " +
        "objectives send and receive items across the multiworld. Free standalone " +
        "download — no Steam required. The built-in AP client connects directly " +
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Download ChecksMate from the GitHub releases page and extract it to " +
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download ChecksMate from the GitHub releases page (link below).",
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title = "Select ChecksMate folder",
        ...
        {
            ("ChecksMate GitHub ↗",   REPO_URL),
            ("ChecksMate releases ↗", REPO_URL + "/releases"),
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "ChecksMate.exe", "checksmate.exe" })
            {
                ...
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        if (Path.GetFileNameWithoutExtension(f)
                                .IndexOf("checks", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch ChecksMate. Make sure the game is installed.", ex);
        }
    }
