...
def main():
    ...
    # A LIST, because a world may accept more than one dump: Pokemon
    # FireRed/LeafGreen names both rev0 and rev1. Read as a plain string this
    # crashed on those patches instead of checking them.
    expected = manifest.get("base_checksum") or []
    ...
    # Replica of the one per-game Python step override we support (worlds may
    # subclass APPatchExtension and change what a step does; see ApPatch.cs).
    # Pokemon FRLG: both steps switch to the rev1 file when the CURRENT rom's
    # revision byte (0xBC) is 1 -- evaluated per step, exactly like rom.py.
    def pick_file(step, requested):
        if manifest.get("game") == "Pokemon FireRed and LeafGreen" \
                and len(rom) > 0xBC and rom[0xBC] == 1:
            if step == "apply_bsdiff4":
                return "base_patch_rev1.bsdiff4"
            if step == "apply_tokens":
                return "token_data_rev1.bin"
        # Pokemon Crystal: revision byte at 332 (the world's AP_ROM_Revision
        # address); only the bsdiff step dispatches. See ApPatch.cs.
        if manifest.get("game") == "Pokemon Crystal" \
                and len(rom) > 332 and rom[332] == 1 and step == "apply_bsdiff4":
            return "basepatch11.bsdiff4"
        return requested

    # Steps that are deliberate no-ops for us: Crystal's apply_overrides reads
    # the PLAYER'S host.yaml option_overrides -- absent any (and we carry no
    # host.yaml), the world's own code returns the rom unchanged.
    def is_noop(step):
        return manifest.get("game") == "Pokemon Crystal" and step == "apply_overrides"

    # An empty procedure is NOT "already patched": APAutoPatchInterface worlds
    # (Pokemon BW, Platinum) patch in their own Python and ship no procedure.
    # Looping over nothing would write the vanilla ROM out as a success.
    if not manifest.get("procedure"):
        fail(4, "patch carries no procedure - this world patches in its own "
                "code, which this tool cannot run")

    bsdiff_source = None
    try:
        for step, args in manifest.get("procedure", []):
            if step == "apply_bsdiff4":
                rom, bsdiff_source = apply_bsdiff4(rom, files[pick_file(step, args[0])], lib_dir)
            elif step == "apply_ips":
                rom = apply_ips(rom, files[args[0]])
            elif step == "apply_tokens":
                rom = apply_tokens(rom, files[pick_file(step, args[0])])
            elif is_noop(step):
                pass
            else:
