...
do
  ...
  -- Append, not truncate. "w" threw away the previous run on every start, so
  -- the log could never explain a session that had already ended -- which is
  -- exactly when anyone goes looking at it.
  log_file = io.open(dir .. "ap_connector.log", "a")
end
