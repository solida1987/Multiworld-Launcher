...
-- Diagnostics. Without these, "no checks are arriving" has three completely
-- different causes that look identical from outside: the poll never runs, it
-- runs but the overworld guard never passes, or it scans fine and the player
-- simply has not tripped a flag yet. The heartbeat below tells them apart.
local stat_polls      = 0           -- poll() calls past the interval gate
local stat_overworld  = 0           --   ...that were in the overworld
local stat_scans      = 0           --   ...that got a clean flag sample
local stat_flags_set  = 0           -- set flags seen in the last clean scan
local stat_last_note  = 0
...
function M.poll()
  ...
  last_poll = now
  stat_polls = stat_polls + 1

  -- Every 30 s, say what the scanner is actually doing. "0 checks" means
  -- something different depending on which of these numbers is stuck.
  if now - stat_last_note >= 30 then
    stat_last_note = now
    log(string.format(
      "scanner: %d polls, %d in overworld, %d clean scans, %d/%d mapped flags set",
      stat_polls, stat_overworld, stat_scans, stat_flags_set, 194))
  end
  ...
  stat_overworld = stat_overworld + 1
    ...
    if byte ~= 0 then
      for bit = 0, 7 do
        if byte % POW2[bit + 1] >= POW2[bit] then
          ...
          if loc then
            seen_set = seen_set + 1
            if not reported[loc] then
              reported[loc] = true
              out[#out + 1] = loc
              log(string.format("CHECK: flag %d -> location %d", flag_id, loc))
            end
          end
