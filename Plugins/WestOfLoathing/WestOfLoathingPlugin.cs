...
namespace LauncherV2.Plugins.WestOfLoathing;

// ═══════════════════════════════════════════════════════════════════════════════
// WestOfLoathingPlugin — native PC, ConnectsItself.
// AP world: "West of Loathing"
// Steam appid: 597220
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class WestOfLoathingPlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────
    private const int STEAM_APPID = 597220;
    ...
    public string GameId           => "west_of_loathing";
    public string DisplayName      => "West of Loathing";
    public string Subtitle         => "Native PC · built-in AP client";
    public string ApWorldName      => "West of Loathing";
    public string IconPath         => Path.Combine(AppContext.BaseDirectory, "Assets", "west_of_loathing.png");
    public string ThemeAccentColor => "#8B6914";
    public string[] GameBadges     => new[] { "Requires Steam" };
    public string Description      =>
        "West of Loathing is a slapstick comedy role-playing game set in the wild west. " +
        "Explore a land of spittoons, cow skulls, and snake oil salesmen while " +
        "fighting skeletons, goblins, and other threats. The Archipelago randomizer " +
        "shuffles the game's items and progress into the multiworld. Requires a Steam copy.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100, "West of Loathing requires manual mod installation. " +
    ...
    public UIElement? CreateSettingsPanel()
    {
        ...
        panel.Children.Add(SectionHeader("WEST OF LOATHING GAME DIRECTORY", muted));
        bool found = IsInstalled;
        panel.Children.Add(new TextBlock
        {
            Text = found
                ? "West of Loathing detected: " + GameDirectory
                : "West of Loathing not found. Install via Steam (appid 597220).",
        ...
        {
            "1. Own and install West of Loathing on Steam (appid 597220).",
            "2. Download the West of Loathing Archipelago mod from the community repository.",
        ...
        {
            ("West of Loathing on Steam ↗", "https://store.steampowered.com/app/597220/West_of_Loathing/"),
    ...
    private void StartGame()
    {
        ...
        if (!string.IsNullOrEmpty(gameDir) && Directory.Exists(gameDir))
            foreach (string name in new[] { "WestOfLoathing.exe", "West of Loathing.exe" })
            {
        ...
        {
            IsRunning = false;
            throw new InvalidOperationException("Could not launch West of Loathing.", ex);
        }
    }
