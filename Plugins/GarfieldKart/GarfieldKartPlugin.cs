...
namespace LauncherV2.Plugins.GarfieldKart;

// ═══════════════════════════════════════════════════════════════════════════════
// GarfieldKartPlugin — launch integration for "Garfield Kart - Furious Racing",
// a Steam racing game with BepInEx AP mod (FeluciaPS/Archipelago community fork).
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * Steam appid 1085510. GameDirectory auto-detected via SteamLocator.
//   * AP game string: "Garfield Kart - Furious Racing"
//   * ConnectsItself = true: BepInEx mod includes the AP client.
//   * SupportsStandalone = true: can launch without an active AP session.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class GarfieldKartPlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const int    STEAM_APPID  = 1085510;
    private const string GH_OWNER    = "FeluciaPS";
    ...
    private const string STEAM_URL   = "https://store.steampowered.com/app/1085510/Garfield_Kart__Furious_Racing/";
    ...
    public string GameId       => "garfield_kart";
    public string DisplayName  => "Garfield Kart - Furious Racing";
    public string Subtitle     => "Steam · BepInEx mod · built-in AP client";
    public string ApWorldName  => "Garfield Kart - Furious Racing";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "garfield_kart.png");

    public string ThemeAccentColor => "#E85010";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "Garfield Kart - Furious Racing is the 2019 racing game featuring " +
        "everyone's favorite lasagna-loving cat. The Archipelago randomizer " +
        "(BepInEx mod) shuffles track unlocks, kart parts, and race wins into " +
        "the multiworld. Requires the Steam version (appid 1085510) with the " +
        "BepInEx mod installed.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Install Garfield Kart - Furious Racing from Steam (appid 1085510), " +
            "then download the BepInEx AP mod from the GitHub releases page and " +
    ...
    // ── AP bridge — inert (BepInEx mod speaks directly to AP server) ─────────
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Own Garfield Kart - Furious Racing on Steam (appid 1085510).",
            "2. Download the BepInEx AP mod from the FeluciaPS/Archipelago fork " +
                "releases (link below) and extract it into the game folder.",
            "3. The game directory is auto-detected from Steam. Click Browse to " +
                "override if needed.",
            "4. Launch from the Play tab. The BepInEx mod will prompt for your " +
                "AP server, slot, and password on first run.",
        })
        {
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title = "Select Garfield Kart - Furious Racing folder",
        ...
        {
            ("AP Mod GitHub (FeluciaPS fork) ↗", REPO_URL),
            ("AP Mod releases ↗",                REPO_URL + "/releases"),
            ("Steam store page ↗",               STEAM_URL),
    ...
    private void StartGame()
    {
        ...
        if (Directory.Exists(dir))
        {
            ...
            {
                "GarfieldKart.exe", "garfieldkart.exe",
                "GarfieldKart_FuriousRacing.exe", "Garfield Kart Furious Racing.exe",
            })
            {
                ...
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        if (Path.GetFileNameWithoutExtension(f)
                                .IndexOf("garfield", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch Garfield Kart - Furious Racing. " +
                "Make sure the game is installed via Steam and the game directory is set correctly.", ex);
        }
    }
