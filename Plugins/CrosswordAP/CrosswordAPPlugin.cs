...
namespace LauncherV2.Plugins.CrosswordAP;

// ═══════════════════════════════════════════════════════════════════════════════
// CrosswordAPPlugin — launch integration for "CrosswordAP", a free standalone
// crossword puzzle Archipelago game (densebamboo/CrosswordAP).
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * No Steam release — free standalone download. GameDirectory = string.Empty
//     default; user sets install dir via Settings.
//   * AP game string: "CrosswordAP"
//   * ConnectsItself = true: built-in AP client handles the server connection.
//   * SupportsStandalone = false: requires an AP room.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class CrosswordAPPlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "densebamboo";
    private const string GH_REPO     = "CrosswordAP";
    ...
    public string GameId       => "crossword_ap";
    public string DisplayName  => "CrosswordAP";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "CrosswordAP";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "crossword_ap.png");

    public string ThemeAccentColor => "#208040";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "CrosswordAP is a crossword puzzle game with Archipelago integration. " +
        "Solve crossword clues to check locations in the multiworld. Free " +
        "standalone download — no Steam required. The built-in client connects " +
        "directly to the AP server.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Download CrosswordAP from the GitHub releases page and extract it to " +
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download CrosswordAP from the GitHub releases page (link below).",
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title = "Select CrosswordAP folder",
        ...
        {
            ("CrosswordAP GitHub ↗",   REPO_URL),
            ("CrosswordAP releases ↗", REPO_URL + "/releases"),
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "CrosswordAP.exe", "crosswordap.exe", "Crossword.exe" })
            {
                ...
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        if (Path.GetFileNameWithoutExtension(f)
                                .IndexOf("crossword", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch CrosswordAP. Make sure the game is installed.", ex);
        }
    }
