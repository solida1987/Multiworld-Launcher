...
namespace LauncherV2.Plugins.Dracomino;

// ═══════════════════════════════════════════════════════════════════════════════
// DracominoPlugin — launch integration for "Dracomino", a free standalone
// Archipelago puzzle game (community AP world).
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * No Steam release — free standalone download. GameDirectory = string.Empty.
//   * AP game string: "Dracomino"
//   * ConnectsItself = true: built-in AP client handles the server connection.
//   * SupportsStandalone = false: designed as an AP-only game.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class DracominoPlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "alwaysintreble";
    private const string GH_REPO     = "Dracomino";
    ...
    public string GameId       => "dracomino";
    public string DisplayName  => "Dracomino";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "Dracomino";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "dracomino.png");

    public string ThemeAccentColor => "#7040C0";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "Dracomino is a free standalone Archipelago puzzle game. Solve puzzles to " +
        "send items across the multiworld. No Steam required — download and play " +
        "for free. The built-in AP client connects directly to the Archipelago server.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Download Dracomino from the GitHub releases page and extract it to " +
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download Dracomino from the GitHub releases page (link below).",
        ...
        {
            ("Dracomino GitHub ↗",    REPO_URL),
            ("Dracomino releases ↗",  REPO_URL + "/releases"),
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "Dracomino.exe", "dracomino.exe" })
            {
                ...
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        if (Path.GetFileNameWithoutExtension(f)
                                .IndexOf("draco", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch Dracomino. Make sure the game is installed.", ex);
        }
    }
