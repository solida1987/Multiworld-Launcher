...
namespace LauncherV2.Plugins.YachtDice;

// ═══════════════════════════════════════════════════════════════════════════════
// YachtDicePlugin — web-browser launch for "Yacht Dice", a browser-based
// Archipelago dice game (similar to Yahtzee / Yatzy) that is bundled directly
// with the main Archipelago distribution.
//
// INTEGRATION MODEL
// ─────────────────
...
//   * LaunchAsync: opens the official Archipelago Yacht Dice page in the default
//     system browser.
//   * GameDirectory = string.Empty: no local files to manage.
//
// BUNDLED AP WORLD
// ────────────────
//   Yacht Dice ships as a bundled world inside the main Archipelago distribution
//   (worlds/yacht_dice/). The exact AP game string is "Yacht Dice" (verified
//   against worlds/yacht_dice/__init__.py in the AP-main repo). There is no
//   separate GitHub release or download to track — AP.gg hosts the web client.
//
// SETUP FLOW
// ──────────
//   1. Generate your multiworld on archipelago.gg with a Yacht Dice slot.
//   2. Click Play — this opens the Yacht Dice client page on archipelago.gg.
//   3. Enter your server, slot name, and password to connect.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class YachtDicePlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string AP_GAME_PAGE = "https://archipelago.gg/games/Yacht%20Dice";
    private const string AP_SITE      = "https://archipelago.gg";

    // ── IGamePlugin — Identity ────────────────────────────────────────────────

    public string GameId        => "yacht_dice";
    public string DisplayName   => "Yacht Dice";
    public string Subtitle      => "Web Browser · built-in AP client";

    /// EXACT AP game string — verified against worlds/yacht_dice/__init__.py
    /// (the World subclass's game = "Yacht Dice") in the AP-main repo.
    public string ApWorldName   => "Yacht Dice";

    public string IconPath =>
        Path.Combine(AppContext.BaseDirectory, "Assets", "yacht_dice.png");

    public string ThemeAccentColor => "#1A6DB5";   // dice blue
    public string[] GameBadges     => new[] { "Free", "Web Browser", "Bundled AP" };

    public string Description =>
        "Yacht Dice is a browser-based Archipelago dice game inspired by Yahtzee / Yatzy. " +
        "Roll dice, fill in your scorecard, and unlock new dice categories as items arrive " +
        "from other worlds. The game is bundled directly with Archipelago and runs entirely " +
    ...
    // Yacht Dice is a bundled AP world served from archipelago.gg — there is no
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((50, "Opening Yacht Dice on archipelago.gg..."));
        OpenUrl(AP_GAME_PAGE);
        progress.Report((100,
            "Yacht Dice is a browser-based game — no installation needed. " +
    ...
    /// Opens the Yacht Dice client page on archipelago.gg in the default browser.
    ...
    public UIElement? CreateSettingsPanel()
    {
        ...
        {
            Text =
                "Yacht Dice is a browser-based Archipelago game inspired by Yahtzee / Yatzy. " +
        ...
        {
            "1. Generate a multiworld on archipelago.gg that includes a Yacht Dice slot.",
            "2. Click Play — your browser will open the Yacht Dice game page.",
        ...
        {
            ("Yacht Dice on archipelago.gg ↗",      AP_GAME_PAGE),
