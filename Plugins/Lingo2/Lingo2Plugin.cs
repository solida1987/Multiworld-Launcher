...
namespace LauncherV2.Plugins.Lingo2;

// ═══════════════════════════════════════════════════════════════════════════════
// Lingo2Plugin — launch integration for "Lingo 2", a standalone word-puzzle
// Archipelago game (community release at code.fourisland.com/lingo2-archipelago).
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * Free standalone download from community release page. GameDirectory =
//     string.Empty default; user sets install dir via Settings.
//   * AP game string: "Lingo 2"
//   * ConnectsItself = true: built-in AP client handles the server connection.
//   * SupportsStandalone = false: requires an AP room.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class Lingo2Plugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string REPO_URL     = "https://code.fourisland.com/lingo2-archipelago/about/";
    private const string RELEASES_URL = "https://code.fourisland.com/lingo2-archipelago/about/";
    ...
    public string GameId       => "lingo2";
    public string DisplayName  => "Lingo 2";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "Lingo 2";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "lingo2.png");

    public string ThemeAccentColor => "#D08020";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "Lingo 2 is the sequel to the Lingo word-puzzle Archipelago game. " +
        "The built-in AP client connects directly. Download from the community " +
        "release page, extract, and browse to the folder.";
...
#pragma warning restore CS0067
    ...
    public async Task CheckForUpdateAsync(CancellationToken ct = default)
    {
        ...
        AvailableVersion = null; // Community Gitea host — no JSON API available
    }
    ...
    {
        progress.Report((100,
            "Download Lingo 2 from the community release page and extract it to " +
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download Lingo 2 from the community release page (link below).",
        ...
        {
            ("Lingo 2 AP World ↗",        REPO_URL),
            ("Lingo 2 releases ↗",        RELEASES_URL),
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "Lingo2.exe", "lingo2.exe", "Lingo 2.exe" })
            {
            ...
            if (exe == null)
            {
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        ...
                        if (stem.IndexOf("lingo", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch Lingo 2. Make sure the game is installed.", ex);
        }
    }
