...
namespace LauncherV2.Plugins.VoidSols;

// ═══════════════════════════════════════════════════════════════════════════════
// VoidSolsPlugin — launch integration for "Void Sols", a minimalist platformer
// with Archipelago randomizer (community fork, cookie966507/Archipelago).
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * Free game with AP-integrated version available on GitHub releases.
//     GameDirectory = string.Empty default; user sets install dir via Settings.
//   * AP game string: "Void Sols"
//   * ConnectsItself = true: built-in AP client handles the server connection.
//   * SupportsStandalone = false: requires an AP room.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class VoidSolsPlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "cookie966507";
    ...
    public string GameId       => "void_sols";
    public string DisplayName  => "Void Sols";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "Void Sols";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "void_sols.png");

    public string ThemeAccentColor => "#6030A0";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "Void Sols is a challenging minimalist platformer. The Archipelago " +
        "randomizer shuffles its upgrades and items into the multiworld. " +
        "The built-in AP client connects directly. Download the AP-integrated " +
        "version from GitHub releases, extract, and browse to the folder.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Download the AP-integrated Void Sols from the GitHub releases page and extract " +
            "it to your preferred folder, then set the path in the Settings panel."));
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download the AP-integrated Void Sols from the GitHub releases page (link below).",
        ...
        {
            ("Void Sols AP GitHub ↗",     REPO_URL),
            ("Void Sols releases ↗",      REPO_URL + "/releases"),
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "VoidSols.exe", "Void Sols.exe",
                                            "void_sols.exe", "voidSols.exe" })
            {
            ...
            if (exe == null)
            {
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        ...
                        if (stem.IndexOf("void", StringComparison.OrdinalIgnoreCase) >= 0 ||
                            stem.IndexOf("sols", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch Void Sols. Make sure the game is installed.", ex);
        }
    }
