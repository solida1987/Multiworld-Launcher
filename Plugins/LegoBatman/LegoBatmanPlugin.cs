...
namespace LauncherV2.Plugins.LegoBatman;

public sealed class LegoBatmanPlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────
    ...
    public string GameId           => "lego_batman";
    public string DisplayName      => "LEGO Batman: The Videogame";
    public string Subtitle         => "Native PC · built-in AP client";
    public string ApWorldName      => "LEGO Batman: The Videogame";
    public string IconPath         => Path.Combine(AppContext.BaseDirectory, "Assets", "lego_batman.png");
    public string ThemeAccentColor => "#1F3D7A";
    public string[] GameBadges     => new[] { "Requires Steam" };

    public string Description =>
        "LEGO Batman: The Videogame is Traveller's Tales' 2008 action-adventure game. " +
        "Play as Batman and Robin — or the villains — through Gotham City and the " +
        "Batcave. In the Archipelago integration, story levels, minikits, and " +
        "gold bricks join the multiworld pool. Requires a Steam copy.";
...
#pragma warning restore CS0067
    ...
    public UIElement? CreateSettingsPanel()
    {
        ...
        panel.Children.Add(new TextBlock
        {
            Text = found ? "LEGO Batman detected: " + GameDirectory
                         : "Game not found. Install via Steam (appid 21000).",
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title            = "Select LEGO Batman game folder",
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "LEGO Batman.exe", "LEGOBatman.exe", "LB.exe" })
            {
