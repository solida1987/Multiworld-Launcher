...
namespace LauncherV2.Plugins.Frogmonster;

// ═══════════════════════════════════════════════════════════════════════════════
// FrogmonsterPlugin — launch integration for "Frogmonster", a free standalone
// Archipelago game (community AP world).
//
// ── FACTS ────────────────────────────────────────────────────────────────────
//   * No Steam release — free standalone download. GameDirectory = string.Empty.
//   * AP game string: "Frogmonster"
//   * ConnectsItself = true: built-in AP client handles the server connection.
//   * SupportsStandalone = false: designed as an AP-only game.
// ═══════════════════════════════════════════════════════════════════════════════

public sealed class FrogmonsterPlugin : IGamePlugin
{
    // ── Constants ────────────────────────────────────────────────────────────

    private const string GH_OWNER    = "alwaysintreble";
    private const string GH_REPO     = "Frogmonster";
    ...
    public string GameId       => "frogmonster";
    public string DisplayName  => "Frogmonster";
    public string Subtitle     => "Native PC · built-in AP client";
    public string ApWorldName  => "Frogmonster";

    public string IconPath
        => Path.Combine(AppContext.BaseDirectory, "Assets", "frogmonster.png");

    public string ThemeAccentColor => "#40A060";

    public string[] GameBadges => Array.Empty<string>();

    public string Description =>
        "Frogmonster is a free standalone Archipelago game. Leap through levels as " +
        "a frog to send items across the multiworld. No Steam required — download " +
        "and play for free. The built-in AP client connects directly to the " +
        "Archipelago server.";
...
#pragma warning restore CS0067
    ...
    {
        progress.Report((100,
            "Download Frogmonster from the GitHub releases page and extract it to " +
    ...
    public System.Windows.UIElement? CreateSettingsPanel()
    {
        ...
        {
            "1. Download Frogmonster from the GitHub releases page (link below).",
        ...
        browseBtn.Click += (_, _) =>
        {
            ...
            {
                Title = "Select Frogmonster folder",
        ...
        {
            ("Frogmonster GitHub ↗",   REPO_URL),
            ("Frogmonster releases ↗", REPO_URL + "/releases"),
    ...
    private void StartGame()
    {
        ...
        {
            foreach (string name in new[] { "Frogmonster.exe", "frogmonster.exe" })
            {
                ...
                try
                {
                                 ...
                                 System.IO.SearchOption.TopDirectoryOnly))
                    {
                        if (Path.GetFileNameWithoutExtension(f)
                                .IndexOf("frog", StringComparison.OrdinalIgnoreCase) >= 0)
                        { exe = f; break; }
                    }
                }
                catch { }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                "Could not launch Frogmonster. Make sure the game is installed.", ex);
        }
    }
